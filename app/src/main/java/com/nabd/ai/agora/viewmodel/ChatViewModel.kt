...
import com.nabd.ai.agora.api.anthropic.*
import com.nabd.ai.agora.api.gemini.*
import com.nabd.ai.agora.api.local.*
import com.nabd.ai.agora.api.ollama.*
import com.nabd.ai.agora.api.openai.*
import com.nabd.ai.agora.data.ClaudeChatImporter
import com.nabd.ai.agora.data.GptChatImporter
...
import com.nabd.ai.agora.data.DataExporter
import com.nabd.ai.agora.data.DataImporter
...
import com.nabd.ai.agora.data.PredefinedVariables
...
import com.nabd.ai.agora.model.AttachmentItem
import com.nabd.ai.agora.model.AttachmentMeta
...
import com.nabd.ai.agora.util.SnackbarEvent
...
import com.nabd.ai.agora.model.SelectedAttachment
...
import com.nabd.ai.agora.viewmodel.delegate.SettingsDelegate
...
class ChatViewModel(
    ...
    init {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            if (settingsManager.autoUpdateCheck.first()) {
                ...
                if (now - lastCheck > 24 * 60 * 60 * 1000L) {
                    ...
                    val info = com.nabd.ai.agora.util.UpdateChecker.check(getCurrentVersion())
                    if (info != null) {
        ...
        // ── Auto Backup ──────────────────────────────────────────
        try { com.nabd.ai.agora.service.AutoBackupWorker.schedule(getApplication()) } catch (_: Exception) {}
        viewModelScope.launch(Dispatchers.IO) {
    ...
    val sandboxManager: com.nabd.ai.agora.sandbox.SandboxManager? by lazy {
        sandboxFactory?.create()
    }
    val isSandboxFlavor: Boolean = sandboxFactory?.isAvailable() == true

    // ── Auto Backup ───────────────────────────────────────────
    val autoBackupManager: com.nabd.ai.agora.data.AutoBackupManager? by lazy {
        try {
            com.nabd.ai.agora.data.AutoBackupManager(getApplication(), settingsManager, chatDao, memoryManager)
    ...
    fun getProviderForModel(modelId: String): String {
        ...
        if (modelId.contains(":")) {
            return com.nabd.ai.agora.model.ModelId.parse(modelId).providerName
        }
    ...
    private val _updateDialogData = MutableStateFlow<com.nabd.ai.agora.util.UpdateInfo?>(null)
    val updateDialogData: StateFlow<com.nabd.ai.agora.util.UpdateInfo?> = _updateDialogData.asStateFlow()
    fun dismissUpdateDialog() { _updateDialogData.value = null }
    fun showUpdateDialog(info: com.nabd.ai.agora.util.UpdateInfo) { _updateDialogData.value = info }
    ...
    private fun resolveTranscriptionModelId(): String =
        imageTranscriptionModel.value?.let { com.nabd.ai.agora.model.ModelId.parse(it).modelName } ?: ""
    ...
    suspend fun checkForUpdates(): com.nabd.ai.agora.util.UpdateInfo? {
        val current = getCurrentVersion()
        return com.nabd.ai.agora.util.UpdateChecker.check(current)
    }
    ...
    fun cacheMessagesForModel(modelId: String, recache: Boolean = false, silent: Boolean = false) {
        ...
        val workRequest = androidx.work.OneTimeWorkRequestBuilder<com.nabd.ai.agora.service.EmbeddingCacheWorker>()
            .setInputData(androidx.work.Data.Builder()
                .putString(com.nabd.ai.agora.service.EmbeddingCacheWorker.KEY_MODEL_ID, modelId)
                .build())
            .addTag(com.nabd.ai.agora.service.EmbeddingCacheWorker.TAG)
            .build()
        androidx.work.WorkManager.getInstance(appContext)
            .enqueueUniqueWork(
                com.nabd.ai.agora.service.EmbeddingCacheWorker.workNameFor(modelId),
    ...
    suspend fun semanticSearch(query: String, limit: Int = 20): List<Pair<MessageEntity, Float>> {
        val ctx = com.nabd.ai.agora.viewmodel.GenerationContext(
    ...
    fun setAutoBackupEnabled(enabled: Boolean) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (enabled) {
                ...
                try { com.nabd.ai.agora.service.AutoBackupWorker.cancel(getApplication()) } catch (_: Exception) {}
            }
        }
    }
    ...
    private fun buildGenerationPair(
        ...
        val config = GenerationConfig(
            ...
            modelId = com.nabd.ai.agora.model.ModelId.parse(modelId).modelName,
            apiKey = activeKey,
        ...
        val genCtx = com.nabd.ai.agora.viewmodel.GenerationContext(
    ...
    fun addShellDevice(device: com.nabd.ai.agora.data.ShellDeviceConfig) {
        viewModelScope.launch {
    ...
    fun updateShellDevice(device: com.nabd.ai.agora.data.ShellDeviceConfig) {
        viewModelScope.launch {
            val current = shellDevices.value.toMutableList()
    ...
    fun generateTitle(conversationId: String) {
        viewModelScope.launch {
            ...
            val modelId = com.nabd.ai.agora.model.ModelId.parse(modelIdWithPrefix).modelName
            val activeKeyId = activeApiKeyIds.value[providerName]
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            for (msg in allMsgs) {
                ...
                if (msg.attachmentMeta != null) {
                    try {
                        val meta = kotlinx.serialization.json.Json.decodeFromString<com.nabd.ai.agora.model.AttachmentMeta>(msg.attachmentMeta!!)
    ...
    fun stopGeneration() {
        com.nabd.ai.agora.api.HttpClient.activeStreamHandle?.cancel()
    ...
    private suspend fun buildEffectiveSystemPrompt(currentId: String): ResolvedPrompt {
        ...
        val modelId = com.nabd.ai.agora.model.ModelId.parse(currentActiveModel.value).modelName
    ...
    fun sendMessage(text: String, images: List<String> = emptyList(), attachments: List<SelectedAttachment> = emptyList()): Boolean {
        ...
        generationJob = generationScope.launch {
            ...
            val metaItems = mutableListOf<com.nabd.ai.agora.model.AttachmentItem>()
            ...
            for (att in attachments) {
                when (att.type) {
                    "image" -> {
                        ...
                        metaItems.add(com.nabd.ai.agora.model.AttachmentItem(
                    ...
                    "pdf" -> {
                        val pagePaths = if (att.preRenderedPaths != null && att.preRenderedPaths.isNotEmpty()) {
                            ...
                            com.nabd.ai.agora.util.PdfPageRenderer.renderAsImages(app, android.net.Uri.parse(att.uri), att.selectedPages)
                        }
            ...
            val attachmentMeta = if (adjustedMetaItems.isNotEmpty()) {
                com.nabd.ai.agora.model.AttachmentMeta(items = adjustedMetaItems)
            } else null
    ...
    fun importClaudeChat(uri: Uri, strategy: com.nabd.ai.agora.ui.settings.ImportStrategy, selectedIds: Set<String>) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (strategy == com.nabd.ai.agora.ui.settings.ImportStrategy.REPLACE) {
    ...
    fun importGptChat(uri: Uri, strategy: com.nabd.ai.agora.ui.settings.ImportStrategy, selectedIds: Set<String>) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
